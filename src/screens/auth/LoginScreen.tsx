  ...
  return (
    <KeyboardAvoidingView style={s.outer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        {/* ── Primary button ── */}
        <TouchableOpacity style={[s.btn, loading && { opacity: 0.7 }]} onPress={signIn} disabled={loading}>
          <Text style={s.btnText}>{loading ? 'Кіруде...' : 'Кіру'}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={s.linkRow} onPress={() => navigation.navigate('Register')}>
          <Text style={s.linkGray}>Аккаунт жоқ па?  </Text>
          <Text style={s.link}>Тіркелу</Text>
        </TouchableOpacity>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  brand: {
    fontSize: 34, fontWeight: '800', color: AMBER, letterSpacing: 5,
    fontFamily: Platform.OS === 'ios' ? 'Georgia' : 'serif',
  },
  tagline: {
    fontSize: 11, color: TEXT_SUB, marginTop: 6,
